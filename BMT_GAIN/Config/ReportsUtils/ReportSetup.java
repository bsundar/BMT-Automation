...
import static ReportsUtils.CustomReport.*;
import static Config.BaseConfig.testOutputPath;
import static Config.BaseConfig.testReportPath;
import static ReportsUtils.Zip.zipFolder;
...
public class ReportSetup implements IReporter {
  private static final Logger L = Logger.getLogger(ReportSetup.class);
...
private int row;
 ...
 // private int m_rowTotal;
  ...
  public void generateReport(List<XmlSuite> xml, List<ISuite> suites, String outdir) {
	...
	try {
		Thread.sleep(5000);
		//Runtime.getRuntime().exec("C:/Program Files/Internet Explorer/iexplore.exe "+testOutputPath+"emailable-report.html") ;
	} catch (Exception e1) {
    ...
    try {
    	...
    	zipFolder (testOutputPath,testReportPath+getdate()+"TestReports.zip");
	} catch (Exception e) {
		
	}
   // GenerateEmailReport.sendEmailReport();
  }
